...
shape = np.shape(glac_features_2d)
updated_glac_features = np.reshape(a = y_pred, newshape = shape)

initial_board = tf.Variable(updated_glac_features)
...
def update_board(X):
    ...
    # Apply rules of the game from Markov Chain algorithm
    X = (N == 3) | (X & (N == 2))
    return X
